...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // 3. Service-role client for admin operations (bypasses RLS for cross-table writes)
  const adminClient = createClient(SUPABASE_URL, SERVICE_ROLE_KEY, {
    auth: { autoRefreshToken: false, persistSession: false },
  });

  // 4. Fetch current bcrypt hash + username from public.users
  ...
  // 5. Verify current password — constant-time bcrypt compare
  ...
  // 6. Bcrypt new password at cost 12 (NFR-9) — NEVER LOG
  let newHash: string;
  try {
  ...
  // 7. Update auth.users password — FIRST (Supabase Auth is authoritative for JWT issuance)
  // Preserve existing app_metadata fields; explicitly set must_change_password=false
  ...
  // 8. Update public.users — sync bcrypt hash + clear must_change_password flag
  const { error: profileUpdateErr } = await adminClient
    .from("users")
    .update({
      bcrypt_password_hash: newHash,
      must_change_password: false,
    })
    .eq("id", actorId)
    .eq("tenant_id", tenantId);
  ...
  if (profileUpdateErr) {
    // auth.users already updated — dual-store is out of sync; log credential_sync_failure
    console.error(JSON.stringify({
      ts: new Date().toISOString(), level: "error",
    ...
    return errorResponse("internal_error", "Password updated but profile sync failed — retry");
  }

  // 9. Log user_events — best-effort; don't fail the response if this fails
  ...
  // 10. Re-sign in with new password to issue fresh JWT with updated claims
  const anonClient = createClient(SUPABASE_URL, ANON_KEY, {
    auth: { autoRefreshToken: false, persistSession: false },
  });

  const { data: newSession, error: signInErr } = await anonClient.auth.signInWithPassword({
    email: userProfile.email_or_username,
    password: newPassword,
  });
