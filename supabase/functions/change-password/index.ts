...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (profileUpdateErr) {
    ...
    // Per deferred-work.md: log credential_sync_failure and fall through — client gets new
    // session. bcrypt_password_hash remains stale until an admin forces a re-sync.
    console.error(JSON.stringify({
      ts: new Date().toISOString(), level: "error",
  ...
  // public.users.email_or_username may differ in casing from the value GoTrue stored.
