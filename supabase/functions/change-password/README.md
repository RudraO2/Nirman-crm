...
1. `auth.users` password via `auth.admin.updateUserById` (Supabase Auth — source of truth for JWT issuance)
2. `public.users.bcrypt_password_hash` (custom bcrypt store used by the `login` Edge Function)

**Order:** auth.users first, then public.users. If public.users update fails, `credential_sync_failure` is logged at severity=error and the client receives a 500. On retry, the user must log in with the new password (auth.users already updated) and use the resulting token to retry the change.

## Credential Logging Guarantee

`currentPassword` and `newPassword` are NEVER logged anywhere — not in structured logs, not in error messages, not in event payloads.

## user_events

On success, inserts `event_type = 'password_changed'` with `actor_id = user_id` (self-initiated). The `payload` carries `{ was_forced_change: boolean }` to distinguish first-login forced changes from self-service changes.
