-- Story 4.4 patch — get_lead_by_id: support shared-lead recipients.
-- Recipients (in lead_shares) previously got 0 rows → 404 on detail screen.
-- Also adds is_shared boolean to return type so detail screen hides Share button.
-- DROP required: RETURNS TABLE shape changes.
...
BEGIN
  ...
  SELECT
    ...
    l.remarks,
    (l.assigned_to_user_id <> v_user_id)::boolean
  FROM public.leads l
...
COMMENT ON FUNCTION public.get_lead_by_id(uuid) IS
  'Story 4.4 (updated) — Single lead fetch with PII decrypted. Returns owned leads AND leads shared with caller. is_shared=true when caller is a recipient. Returns 0 rows if not found or not accessible.';
